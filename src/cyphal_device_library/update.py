...
async def update_all_nodes(client: Client, software_files: SoftwareDirectory, console: rich.console.Console) -> None:
    ...
    while True:
        response = await loop.run_in_executor(None, console.input, "Continue with update? \\[y/n] ")
...
async def main(parallel_updates: int = 1):
    console = rich.console.Console()

    software_files = SoftwareDirectory.from_path("bin")
...
if __name__ == "__main__":
    ...
    n_parallel = 12 if "socketcan" in os.environ.get("UAVCAN__CAN__IFACE", "") else 1

    try:
        asyncio.run(main(n_parallel))
